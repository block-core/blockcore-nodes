...
namespace Blockcore.Networks.Xds.Configuration
{
   ...
   public class XdsPreMempoolChecksMempoolRule : MempoolRule
   {
          ...
          ILoggerFactory loggerFactory) : base(network, txMempool, mempoolSettings, chainIndexer, loggerFactory)
      {
         ...
         checkPowTransactionRule = consensusRules.GetRule<CheckPowTransactionRule>();
      }
      ...
      public override void CheckTransaction(MempoolValidationContext context)
      {
         ...
         {
            logger.LogTrace("(-)[FAIL_INVALID_COINBASE]");
         ...
         {
            logger.LogTrace("(-)[FAIL_INVALID_COINSTAKE]");
            context.State.Fail(MempoolErrors.Coinstake).Throw();
         }

         checkPowTransactionRule.CheckTransaction(network, network.Consensus.Options, transaction);
         ...
         if (!CheckFinalTransaction(chainIndexer, dateTimeProvider, context.Transaction))
         {
            logger.LogTrace("(-)[FAIL_NONSTANDARD]");
      ...
      private void CheckStandardTransaction(MempoolValidationContext context)
      {
         ...
         if (tx.Version > network.Consensus.Options.MaxStandardVersion || tx.Version < 1)
         {
            logger.LogTrace("(-)[FAIL_TX_VERSION]");
         ...
         foreach (TxOut txOut in tx.Outputs)
         {
            ...
            {
               logger.LogTrace("(-)[FAIL_EMPTY_SCRIPTPUBKEY]");
            ...
            if (txOut.IsDust(nodeSettings.MinRelayTxFeeRate))
            {
               logger.LogTrace("(-)[FAIL_DUST]");
         ...
         if (dataOut > 1)
         {
            logger.LogTrace("(-)[FAIL_MULTI_OPRETURN]");
